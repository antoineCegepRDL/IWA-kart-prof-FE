...
import KartProductComponent from '#components/User/TheKartProduct';
...
const KartPage = () => {
  ...
  const calculateNewTotal = (products: Product[]) => {
    const total = products.reduce((acc, x) => acc + x.quantityToBuy * ((1 - x.discountPercentage) * x.price), 0);
    setTotal(total);
  };
          ...
          <div className="list">
            {products.map((product) => (
              <div className="product-kart">
                <KartProductComponent
                  product={product}
                  onChangeQuatity={onChangeQuatity}
